...
function Compiler:emitContainerFuncBody()
    ...
    local function buildWhileBody(tb, l, r, pScope, scope)
        ...
        -- Use a slightly randomized split point instead of always ceil(len/2)
        -- This breaks the perfectly balanced binary tree fingerprint
        local splitBias = math.random(0, 1);
        local mid = l + math.floor(len / 2) + splitBias;
        if mid > r then mid = r end
        if mid <= l then mid = l + 1 end
