...
local Tokenizer = require("ZukaTech.tokenizer");
...
function Parser:statement(scope, currentLoop)
	...
	if expr then
		...
		if(expr.kind == AstKind.IndexExpression or expr.kind == AstKind.VariableExpression) then
			...
			-- Compound Assignment (LuaU only, but detect in all modes for a clear error)
			local compoundOps = {"+=", "-=", "*=", "/=", "%=", "^=", "..="};
			...
			if isCompound then
				if(self.luaVersion ~= LuaVersion.LuaU) then
					if self.disableLog then error() end;
					logger:error(generateError(self, "compound assignment operators (+=, -=, etc.) are only supported in LuaU mode. Use Lua51 = false or switch to LuaU in your obfuscator settings"));
				end
